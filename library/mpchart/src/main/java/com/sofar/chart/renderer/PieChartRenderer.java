package com.sofar.chart.renderer;
...
import com.sofar.chart.data.PieData;
import com.sofar.chart.data.PieDataSet;
import com.sofar.chart.data.PieEntry;
import com.sofar.chart.formatter.IValueFormatter;
import com.sofar.chart.highlight.Highlight;
import com.sofar.chart.interfaces.datasets.IPieDataSet;
import com.sofar.chart.utils.ColorTemplate;
import com.sofar.chart.utils.MPPointF;
import com.sofar.chart.utils.Utils;
import com.sofar.chart.utils.ViewPortHandler;
...
public class PieChartRenderer extends DataRenderer {
  ...
  /**
   * Calculates the sliceSpace to use based on visible values and their size compared to the
   * set sliceSpace.
   *
   * @param dataSet
   * @return
  ...
  protected void drawDataSet(Canvas c, IPieDataSet dataSet) {
    ...
    for (int j = 0; j < entryCount; j++) {
      ...
      if (drawRoundedSlices) {
        float x =
          center.x + (radius - roundedRadius) * (float) Math.cos(startAngleOuter * Utils.FDEG2RAD);
        float y =
          center.y + (radius - roundedRadius) * (float) Math.sin(startAngleOuter * Utils.FDEG2RAD);
        roundedCircleBox
          .set(x - roundedRadius, y - roundedRadius, x + roundedRadius, y + roundedRadius);
      }
      ...
      if (drawInnerArc && (innerRadius > 0.f || accountForSliceSpacing)) {
        ...
        if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= Utils.FLOAT_EPSILON) {
          ...
          if (drawRoundedSlices) {
            float x = center.x +
              (radius - roundedRadius) * (float) Math.cos(endAngleInner * Utils.FDEG2RAD);
            float y = center.y +
              (radius - roundedRadius) * (float) Math.sin(endAngleInner * Utils.FDEG2RAD);
            roundedCircleBox
              .set(x - roundedRadius, y - roundedRadius, x + roundedRadius, y + roundedRadius);
    ...
    if (mChart.isDrawHoleEnabled() && mBitmapCanvas != null) {
      ...
      if (Color.alpha(mTransparentCirclePaint.getColor()) > 0 &&
        ...
        mTransparentCirclePaint
          .setAlpha((int) ((float) alpha * mAnimator.getPhaseX() * mAnimator.getPhaseY()));
  ...
  public void drawHighlighted(Canvas c, Highlight[] indices) {
     ...
     * in this way.
     * TODO: add support for changing slice color with highlighting rather than only shifting the
     *   slice
     */

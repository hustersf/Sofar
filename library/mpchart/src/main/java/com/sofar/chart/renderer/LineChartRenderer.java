...
public class LineChartRenderer extends LineRadarRenderer {
  ...
  protected void drawCubicBezier(ILineDataSet dataSet) {
    ...
    if (mXBounds.range >= 1) {
      ...
      // That's because we need 4 points for a cubic bezier (cubic=4), otherwise we get lines
      // moving and doing weird stuff on the edges of the chart.
  ...
  protected void drawLinear(Canvas c, ILineDataSet dataSet) {
    ...
    if (dataSet.getColors().size() > 1) {
      ...
      if (mLineBuffer.length <
        ...
        mLineBuffer =
          new float[Math.max((entryCount) * pointsPerEntryPair, pointsPerEntryPair) * 4];
      }
  ...
  public void drawValues(Canvas c) {
    ...
    if (isDrawingValuesAllowed(mChart)) {
      ...
      for (int i = 0; i < dataSets.size(); i++) {
        ...
        float[] positions =
          trans.generateTransformedValuesLine(dataSet, mAnimator.getPhaseX(), mAnimator
  ...
  private class DataSetImageCache {
    ...
    protected void fill(ILineDataSet set, boolean drawCircleHole,
      ...
      for (int i = 0; i < colorCount; i++) {
        ...
        Bitmap circleBitmap =
          Bitmap.createBitmap((int) (circleRadius * 2.1), (int) (circleRadius * 2.1), conf);

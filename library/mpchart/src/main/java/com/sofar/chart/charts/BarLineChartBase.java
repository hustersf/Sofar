...
import com.sofar.chart.components.XAxis.XAxisPosition;
...
import com.sofar.chart.highlight.ChartHighlighter;
import com.sofar.chart.highlight.Highlight;
import com.sofar.chart.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
import com.sofar.chart.interfaces.datasets.IBarLineScatterCandleBubbleDataSet;
import com.sofar.chart.jobs.AnimatedMoveViewJob;
import com.sofar.chart.jobs.AnimatedZoomJob;
import com.sofar.chart.jobs.MoveViewJob;
import com.sofar.chart.jobs.ZoomJob;
import com.sofar.chart.listener.BarLineChartTouchListener;
import com.sofar.chart.listener.OnDrawListener;
import com.sofar.chart.renderer.XAxisRenderer;
import com.sofar.chart.renderer.YAxisRenderer;
import com.sofar.chart.utils.MPPointD;
import com.sofar.chart.utils.MPPointF;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
  ...
  protected void init() {
    ...
    mChartTouchListener =
      new BarLineChartTouchListener(this, mViewPortHandler.getMatrixTouch(), 3f);
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mXAxis.isDrawAxisLineBehindGrid()) {
      mXAxisRenderer.renderAxisLine(canvas);
    }
    if (mAxisLeft.isDrawAxisLineBehindGrid()) {
      mAxisRendererLeft.renderAxisLine(canvas);
    }
    if (mAxisRight.isDrawAxisLineBehindGrid()) {
  ...
  public void zoom(float scaleX, float scaleY, float xValue, float yValue, AxisDependency axis) {
    ...
    Runnable job = ZoomJob.getInstance(mViewPortHandler, scaleX, scaleY, xValue, yValue,
   ...
   * minRange=10, maxRange=100 the
   * smallest range to be displayed at once is 10, and no more than a range of 100 values can be
   * viewed at once without
   * scrolling
   *
   * @param minXRange
   * @param maxXRange
  ...
  /**
   * When disabled, the data and/or highlights will not be clipped to contentRect. Disabling
   * this option can
   * be useful, when the data lies fully within the content rect, but is drawn in such a way
   * (such as thick lines)
  ...
  /**
   * Sets whether the chart should keep its position (zoom / scroll) after a rotation
   * (orientation change)
   */

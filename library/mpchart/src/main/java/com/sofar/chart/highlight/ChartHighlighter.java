...
public class ChartHighlighter<T extends BarLineScatterCandleBubbleDataProvider>
  ...
  protected Highlight getHighlightForX(float xVal, float x, float y) {
    ...
    YAxis.AxisDependency axis =
      leftAxisMinDist < rightAxisMinDist ? YAxis.AxisDependency.LEFT : YAxis.AxisDependency.RIGHT;

    Highlight detail =
      getClosestHighlightByPixel(closestValues, x, y, axis, mChart.getMaxHighlightDistance());
   ...
   * The returned list contains two objects per DataSet (closest rounding up, closest rounding
   * down).
   *
   ...
   * y-axis.
   *
   * @param closestValues        contains two Highlight objects per DataSet closest to the
   *                             selected x-position (determined by
   *                             rounding up an down)
   * @param x
   * @param y

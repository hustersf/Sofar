...
import com.sofar.ui.R;

/**
 * 可定制阴影颜色的CardView
 * but 阴影会占据view大小
 */
public class CardViewEx extends FrameLayout {

  private static final int[] COLOR_BACKGROUND_ATTR = {android.R.attr.colorBackground};
  private static final CardViewExImpl IMPL;

  static {
    if (Build.VERSION.SDK_INT >= 17) {
      IMPL = new CardViewExApi17Impl();
    } else {
      IMPL = new CardViewExBaseImpl();
    }
    IMPL.initStatic();
  }
  ...
  public CardViewEx(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CardViewEx, defStyleAttr,
      R.style.CardViewExDefault);
    ColorStateList backgroundColor;
    if (a.hasValue(R.styleable.CardViewEx_cardBackgroundColor)) {
      backgroundColor = a.getColorStateList(R.styleable.CardViewEx_cardBackgroundColor);
    } else {
    ...
    float radius = a.getDimension(R.styleable.CardViewEx_cardCornerRadius, 0);
    float elevation = a.getDimension(R.styleable.CardViewEx_cardElevation, 0);
    float maxElevation = a.getDimension(R.styleable.CardViewEx_cardMaxElevation, 0);
    int shadowStartColor = a.getColor(R.styleable.CardViewEx_cardShadowStartColor, 0);
    int shadowEndColor = a.getColor(R.styleable.CardViewEx_cardShadowEndColor, 0);
    mCompatPadding = a.getBoolean(R.styleable.CardViewEx_cardUseCompatPadding, false);
    mPreventCornerOverlap = a.getBoolean(R.styleable.CardViewEx_cardPreventCornerOverlap, true);
    int defaultPadding = a.getDimensionPixelSize(R.styleable.CardViewEx_contentPadding, 0);
    mContentPadding.left = a.getDimensionPixelSize(R.styleable.CardViewEx_contentPaddingLeft,
      defaultPadding);
    mContentPadding.top = a.getDimensionPixelSize(R.styleable.CardViewEx_contentPaddingTop,
      defaultPadding);
    mContentPadding.right = a.getDimensionPixelSize(R.styleable.CardViewEx_contentPaddingRight,
      defaultPadding);
    mContentPadding.bottom = a.getDimensionPixelSize(R.styleable.CardViewEx_contentPaddingBottom,
      defaultPadding);
    if (elevation > maxElevation) {
      maxElevation = elevation;
    }
    mUserSetMinWidth = a.getDimensionPixelSize(R.styleable.CardViewEx_android_minWidth, 0);
    mUserSetMinHeight = a.getDimensionPixelSize(R.styleable.CardViewEx_android_minHeight, 0);
    a.recycle();
  ...
  private final CardViewExDelegate mCardViewDelegate = new CardViewExDelegate() {
    ...
    @Override
    public boolean getUseCompatPadding() {
      return CardViewEx.this.getUseCompatPadding();
    }

    @Override
    public boolean getPreventCornerOverlap() {
      return CardViewEx.this.getPreventCornerOverlap();
    }

    @Override
    public void setShadowPadding(int left, int top, int right, int bottom) {
      mShadowBounds.set(left, top, right, bottom);
      CardViewEx.super.setPadding(left + mContentPadding.left, top + mContentPadding.top,
    ...
    public void setMinWidthHeightInternal(int width, int height) {
      if (width > mUserSetMinWidth) {
        CardViewEx.super.setMinimumWidth(width);
      }
      if (height > mUserSetMinHeight) {
        CardViewEx.super.setMinimumHeight(height);
      }
    }

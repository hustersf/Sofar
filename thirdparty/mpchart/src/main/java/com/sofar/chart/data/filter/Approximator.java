...
package com.sofar.chart.data.filter;
...
public class Approximator {
  ...
  public float[] reduceWithDouglasPeucker(float[] points, float tolerance) {
    ...
    Line line =
      new Line(points[0], points[1], points[points.length - 2], points[points.length - 1]);
    ...
    if (greatestDistance > tolerance) {
      ...
      float[] reduced1 =
        reduceWithDouglasPeucker(Arrays.copyOfRange(points, 0, greatestIndex + 2), tolerance);
      float[] reduced2 =
        reduceWithDouglasPeucker(Arrays.copyOfRange(points, greatestIndex, points.length),
          tolerance);
